    ...
    try:
        with open(filename, 'rb') as file:
            ...
            while True:
                ...
                # Για testing - αφαίρεσε αυτό το limit για πλήρη dataset
                # if len(data) >= 1000:
                #    break
...
def return_data(input_file: str, data_type: str) -> List[List[float]]:  # Αλλαγή: type -> data_type
    """
    Συνάρτηση για ανάγνωση MNIST training/test data
    
    Args:
        input_file: Το path του αρχείου
        data_type: "sift" ή "mnist"
    """
   
    if data_type == "sift":  # Αλλαγή: type -> data_type
        data = read_sift_file(input_file)
    elif data_type == "mnist":  # Αλλαγή: type -> data_type
        data = read_mnist_file(input_file)
    else:
        raise ValueError(f"Unknown data type: {data_type}. Use 'sift' or 'mnist'")
